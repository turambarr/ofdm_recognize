function main_detect_ofdm
    % --- 1. 用户必须配置的参数 ---
    FILE_PATH = 'starlink_ku_band_signal.dat'; % 替换为您的文件路径
    SAMPLE_RATE = 240e6; % 替换为您的真实采样率 (Hz)
    SAMPLES_TO_READ = 4e6; % 要读取和分析的采样点总数 (I/Q样本)
...
function iq_samples = read_iq_dat(file_path, num_samples)
    ...
    % 读取 int16 数据，I 和 Q 是交错的，所以要读取 2 * num_samples 个值
    data_raw = fread(fid, num_samples * 2, 'int16');
